...
public class TradeTransactionServiceImpl implements TradeTransactionService {
    ...
    public boolean saveTradeTransaction(SaveTradeTransaction tradeTransaction) {
        ...
        //获取买方/卖方信息
        var buyer = userDao.selectUserAccountForUpdate(tradeTransaction.getBuyerId());
        var buyerGood = userGoodDao.selectUserCoinAccountForUpdate(buyer.getUserId());
        if (buyer == null || buyerGood == null) {
            throw new SimpleException("买方账户异常");
        }

        var seller = userDao.selectUserAccountForUpdate(tradeTransaction.getSellerId());
        var sellerGood = userGoodDao.selectUserCoinAccountForUpdate(seller.getUserId());
        if (seller == null || sellerGood == null) {
            throw new SimpleException("卖方账户异常");
        }
        ...
        var addMoney =
                buyerOrder.getTradePrice().subtract(tradeTransaction.getTradePrice()).multiply(tradeTransaction.getTradeCount());
